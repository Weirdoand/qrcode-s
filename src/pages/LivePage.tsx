...
export default function LivePage() {
  ...
  useEffect(() => {
    ...
    api.getLiveCode(slug)
      .then(c => {
        if (c.type === 'url') { window.location.replace(c.target); return; }
        setCode(c);
        setError('done');
      })
  ...
  if (error === 'not_found') return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center gap-4 text-center px-4">
      ...
      <h1 className="text-white text-2xl font-bold">{t('live.notFoundTitle')}</h1>
      <p className="text-slate-400 text-sm">{t('live.notFoundDesc')}</p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center px-4">
      ...
      <div className="w-full max-w-sm">
        <img src={code.target} alt={code.title} className="w-full rounded-2xl shadow-2xl object-contain" style={{ maxHeight: '75vh' }} />
      </div>
      <p className="mt-6 text-slate-600 text-xs text-center">{t('live.longPress')}</p>
      <div className="mt-4 text-slate-700 text-xs font-mono">liveqr · /{slug}</div>
    </div>
  );
}