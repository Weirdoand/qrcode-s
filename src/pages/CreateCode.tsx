...
export default function CreateCode() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      toast.success('Live code created!');
      navigate('/dashboard');
    } catch (err) {
      const msg = err instanceof Error ? err.message : 'Something went wrong';
      if (msg === 'limit_exceeded') {
        toast.error('You have reached the 20 code limit');
      } else {
        toast.error(msg);
      }
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      <header className="px-5 py-4 border-b border-white/5 flex items-center gap-4 bg-black/20 backdrop-blur-md sticky top-0 z-20">
        ...
        <span className="text-white font-semibold">New Live Code</span>
      </header>
      ...
      <main className="flex-1 max-w-lg mx-auto w-full px-4 py-10">
        <form onSubmit={handleSubmit} className="space-y-7">
          ...
          <div className="space-y-2">
            ...
            <Input
              placeholder="e.g. Team A Group Chat"
              value={title}
          ...
          {/* Description */}
          <div className="space-y-2">
            <Label className="text-xs uppercase tracking-widest text-slate-400 font-semibold">Description <span className="text-slate-600 normal-case">(optional)</span></Label>
            <Input
              placeholder="e.g. Updated whenever the invite expires"
              value={description}
          ...
          {/* Image upload */}
          <div className="space-y-2">
            <Label className="text-xs uppercase tracking-widest text-slate-400 font-semibold">Image *</Label>

            {!file ? (
              <div
                className={`border-2 border-dashed rounded-xl p-10 flex flex-col items-center justify-center gap-3 cursor-pointer transition-colors ${dragOver ? 'border-sky-400 bg-sky-400/5' : 'border-white/10 hover:border-white/25 hover:bg-white/2'}`}
              ...
              >
                ...
                <div className="text-center">
                  <p className="text-white font-medium">Drop image here or <span className="text-sky-400">browse</span></p>
                  <p className="text-slate-500 text-sm mt-1">JPEG, PNG, WEBP · max 5 MB</p>
                </div>
                <input
                  ref={inputRef}
                  type="file"
          ...
          <Button
            type="submit"
            disabled={uploading}
            className="w-full h-11 bg-sky-400 text-slate-950 hover:bg-sky-300 font-bold"
          >
            {uploading ? (
              <><Loader2 className="h-4 w-4 mr-2 animate-spin" /> Creating…</>
            ) : (
              <><Upload className="h-4 w-4 mr-2" /> Create Live Code</>
            )}
          </Button>
        </form>
      </main>
